...
const enabled_cetech_modules: []const []const u8 = &(cetech1_build.core_modules ++ cetech1_build.editor_modules);
...
pub fn build(b: *std.Build) !void {
    ...
    const external_credits: []const std.Build.LazyPath = &.{b.path(".external_credits.json")};
    ...
    const exe = b.addExecutable(.{
        .name = BIN_NAME,
        .version = version,
        .root_source_file = b.path("externals/cetech1/src/main.zig"),
        .target = target,
        .optimize = optimize,
    });
    exe.linkLibC();
    b.installArtifact(exe);
    cetech1_build.useSystemSDK(b, target, exe);
    cetech1_build.createRunStep(b, exe);
    ...
    gen_ide.addDirectoryArg(b.path(".generate_ide.json"));

    gen_ide_step.dependOn(&gen_ide.step);

    exe.root_module.addImport("kernel", cetech1.module("kernel"));
    exe.linkLibrary(cetech1.artifact("cetech1_static"));
